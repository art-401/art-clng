...
def main():
    os.system('clear')
    print logo
    jalan('  This is Only for educational purpose i am not responsible for any mess use:) thank you')
    jalan('  ART MAIN MENU')
    print ' \x1b[1;97m    [ART] MAIN MENU'
    print '\x1b[1;97m-----------------------------------------------------'
...
def mawan():
    ...
    if kontol in ('open', 'Open', 'OPEN'):
        print '\n%s *%s Download watch video for token non expired Use kiwi browser' % (B, N)
        time.sleep(2)
        raw_input(' %s*%s tekan enter ' % (O, N))
        os.system('xdg-open https://www.mediafire.com/file/r6l84jeatnu991t/ART+COMMAND.mp4/file')
        mawan()
